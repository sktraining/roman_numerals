...
describe 'csv converter' do
  ...
  describe '#convert_csv_at_path' do
  ...
  describe '#convert_csv' do
    context 'csv file is valid' do
      context 'roman to arabic case' do
        ...
        it 'csv file contains converted arabic values' do
          expect(converter.convert_csv(input_csv_string)).to eq(output_csv)
        end
      end

      context 'arabic to roman case' do
        let(:converter) { CSVConverter.new(ArabicToRomanConverter) }
        let(:input_csv_string) { "10\n4\n109\n1000" }
        let(:input_csv_array) { ['10', '4', '109', '1000'] }
        let(:output_csv_array) { ['X', 'IV', 'CIX', 'M'] }
        let(:output_csv) do
          CSV.generate do |csv|
    ...
    context 'csv file is invalid' do
      ...
      context "row has more than one cell" do
        let(:invalid_input_csv_string) { "I,X\nIV,M" }
        specify { expect { converter.convert_csv(invalid_input_csv_string) }.to raise_error("input must be a valid csv file") }
      end

      context "row is nil" do
        specify { expect { converter.convert_csv(nil) }.to raise_error("input must be a valid csv file") }
      end
    end
  end
end
